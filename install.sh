...
echo "📦 安装到 $INSTALL_DIR（可能需要输入密码）..."
sudo cp .build/release/nosleepclub "$INSTALL_DIR/nosleepclub"
sudo chmod +x "$INSTALL_DIR/nosleepclub"
sudo codesign --force --sign - "$INSTALL_DIR/nosleepclub" 2>/dev/null

echo ""
echo "✅ 安装完成！打开任意终端直接运行："
echo ""
echo "  nosleepclub          # 启动（然后可以合盖）"
echo "  nosleepclub &        # 后台运行"
echo "  pkill nosleepclub    # 停止"
echo "  Ctrl+C               # 退出"
echo ""
